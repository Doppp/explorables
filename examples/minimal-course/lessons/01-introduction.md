...
Move the input slider. The output is always twice the selected input, so an input of 7 produces 14. The visible equation updates with the same values.
:::

## Implement it

:::exercise{path="../exercises/double" command="pnpm test" title="Implement double"}
Implement a pure function that doubles finite numbers, then inspect the failing test for non-finite input.
:::

## Explain

Why might rejecting `Infinity` be preferable to returning `Infinity` in a larger numeric pipeline?
