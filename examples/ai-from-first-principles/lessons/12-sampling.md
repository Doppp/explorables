...
A model produces logits, not a finished answer. A decoding policy reshapes and truncates their distribution before selecting a token.
...
Enable the broken top-p order. Change logits so the highest probability is not the first token; the kept set now depends on vocabulary position.
...
Why can lowering temperature and lowering top-p both reduce variety while producing different distributions? To compare two cache implementations, why would a greedy or seeded decoding policy be more informative than unrelated random samples?
