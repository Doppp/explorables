...
One attention operation produces one mixing pattern. Multi-head attention first
projects the residual stream, splits its width, lets each head mix the sequence
independently, concatenates the results, and projects them back to the model
width.

> **Predict:** A model width of 4 is split into 2 heads. How wide is each head?
...
making their weights identical and discarding the second slice.
:::

## Inspect independent mixtures

Change only the last two features of a token. Head 1 is unchanged while Head 2
can change, because the heads receive different feature slices in this toy
projection. Turn off the causal mask and identify the newly visible future
positions.
...
Split features into contiguous heads, scale query-key scores, mask before
softmax, mix values, and concatenate the head outputs in their original order.
...
projection play after independent heads have produced their mixtures?
