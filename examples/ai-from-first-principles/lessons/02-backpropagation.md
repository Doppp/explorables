...
Our graph computes `z = xw + b`, then `y = z²`. Backpropagation reuses the forward value of `z` while multiplying local derivatives backward.
...
Toggle the broken gradient. Compare its value with a finite-difference check; the error grows with `x` because the missing local derivative is `dz/dw = x`.
...
Why is caching `z` useful during the backward pass? Which cached values would you need if `y = tanh(z)` instead?
