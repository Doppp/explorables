...
A pre-norm Transformer block does not replace its representation twice. It normalises a view of the residual stream, computes an attention update and adds it, then normalises again, computes a gated MLP update and adds that too.
...
Set the attention update scale to zero. The residual stream still reaches the MLP. In broken replacement mode, the zero attention result erases the original token before the MLP sees it.

The attention update in this explorable represents the already-computed result for one selected token; the previous lesson supplied the sequence mixing.
...
Why is an unchanged residual path useful for gradient flow through many blocks? Why does RMSNorm preserve the sign pattern of a constant non-zero vector while mean-centred normalisation removes it?
